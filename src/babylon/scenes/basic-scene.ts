...
import {coordinates, coordinatesWithInteraction, fencesCoodinates, lampCoordinates} from "../lib/constants.ts";
...
export class BasicScene extends BaseScene {
    ...
    importAvatar = ({scene, smallLight}: { scene: Scene, smallLight: any }) => {
        SceneLoader.ImportMesh(
            ...
            (newMeshes, _, skeletons) => {
                ...
                this.main.position = new Vector3(8, 0, 10);
            },
            () => {
            }
        );
    ...
    createScene = async ({engine, setIsShowModal, setSelectedObjectName, setIsShowInfoModal}: {
        ...
        document.onkeydown = (evt: any) => {
            ...
            if (isPressedE && this.existCollision && (['mintPass', 'Chain', 'shiba']?.includes(this.collisionWith?.name) || this.collisionWith?.name?.includes('building'))) {
    ...
    createImagePlane = (xp: any, yp: any, zp: any, xr: any, yr: any, zr: any, scene: Scene, shadowGenerator: ShadowGenerator, image = './back.jpg', id: any) => {
        console.log(id)
        try {
            const img = new Image();
            img.src = image;
            img.onload = function () {
                ...
                const plane = MeshBuilder.CreatePlane("plane1" + xp + yp + zp, {
                    height: 3,
                    width: Math.min(3 * width / height, 4)
                }, scene);
                const mat = new StandardMaterial("plane2", scene);
                ...
                const box = MeshBuilder.CreateBox("box23", {height: 10, width: 4, depth: 0.26});
                box.isPickable = true
